...
const statusConfig: Record<string, { bg: string, text: string, label: string }> = {
  ...
  working: { bg: 'bg-yellow-900/20', text: 'text-aic-yellow', label: 'WORKING' },
...
export const WorkerDesk = memo(function WorkerDesk({ worker, status, engine, subWorkerCount, subWorkerTotal }: WorkerDeskProps) {
  ...
  const cardStyle = status === 'working' ? 'border-aic-yellow shadow-[0_0_15px_rgba(255,204,0,0.3)] bg-yellow-900/10'
    ...
    : 'border-gray-600/50 shadow-[0_0_15px_rgba(107,114,128,0.2)] bg-gray-800/20'; // idle (soft blue pulse)
  ...
  return (
    <div className={`w-[175px] h-[160px] relative rounded-lg border-2 flex flex-col items-center pt-3 pb-2 transition-all duration-1000 animate-pulse-slow ${cardStyle}`}>
      ...
      <div className="h-[28px] w-full flex flex-col items-center justify-center shrink-0 mt-2">
